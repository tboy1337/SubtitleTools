...
def _add_translate_parser(subparsers: "Any") -> None:
...
def _add_encode_parser(subparsers: "Any") -> None:
...
def handle_transcribe_command(args: argparse.Namespace) -> int:  # pylint: disable=too-many-return-statements
    ...
    try:
        transcriber = SubWhisperTranscriber(args.model, args.language)
...
def handle_translate_command(args: argparse.Namespace) -> int:  # pylint: disable=too-many-return-statements
    ...
    try:
        translator = SubtitleTranslator(args.service, args.api_key)
        ...
        if args.batch or os.path.isdir(args.input):
            ...
            for file_path in files:
                try:
                    ...
                    # Parse subtitles
                    subtitles = subtitle_processor.parse_file(file_path, args.encoding)
                    ...
                    space = is_space_language(args.target_lang)
                    ...
                    subtitle_processor.save_file(translated_subtitles, output_path, args.encoding)
                    successful += 1
        ...
        # Parse subtitles
        subtitles = subtitle_processor.parse_file(input_path, args.encoding)
...
def handle_encode_command(args: argparse.Namespace) -> int:  # pylint: disable=too-many-return-statements
    ...
    try:
        ...
        if args.to_encoding:
            ...
            print(f"Using recommended encodings for '{args.language}': {', '.join(target_encodings)}")
        else:
