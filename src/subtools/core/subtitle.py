...
class SubtitleProcessor:
    ...
    def split_sentences(
        ...
        if language_code and language_code.startswith("zh") and JIEBA_AVAILABLE:
            ...
            try:
                segments_generator = jieba.cut(all_text, cut_all=False)
                sentences = list(segments_generator)
    ...
    def reconstruct_subtitles(
        ...
        reconstructed: List[srt.Subtitle] = []
    ...
    def merge_subtitles(
        ...
        for i in range(min_length):
            ...
            merged_subtitle = srt.Subtitle(
                ...
                proprietary=getattr(sub1, 'proprietary', "")
            )
        ...
        if len(subtitles1) > min_length:
            ...
            for i in range(min_length, len(subtitles2)):
                ...
                adjusted_subtitle = srt.Subtitle(
                    index=len(merged) + 1,
                    start=sub2.start,
                    end=sub2.end,
                    content=sub2.content,
                    proprietary=getattr(sub2, 'proprietary', "")
                )
