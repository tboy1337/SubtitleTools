        ...
        **whisper_options: Any,
        ...
        try:
            ...
            if max_segment_length:
                segments = self._split_long_segments(segments, max_segment_length)

            return WhisperResult(segments=segments, language=detected_language)
    ...
    def _perform_transcription(
        ...
        try:
            ...
            if audio_data.dtype == np.int16:
                ...
                logger.debug("Audio data already in format: %s", audio_data.dtype)

            # Convert to mono if stereo
            if audio_data.ndim > 1:
                logger.debug("Converting stereo to mono (shape: %s)", audio_data.shape)
            ...
            logger.debug("Final audio shape: %s", audio_data.shape)
    ...
    def get_model_info(self) -> Dict[str, Union[str, Optional[str], bool]]:
        ...
        if self._model is not None:
            try:
                ...
                dims = getattr(self._model, "dims", {})
                if hasattr(dims, "get"):
                    info["model_size"] = dims.get("n_mels", "unknown")
                else:
